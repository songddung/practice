...
def collect_and_analyze_books():
    ...
    for topic in topics:
        books = fetch_books_by_topic(topic)['item']
    ...
    #     # 5.3 [ 주제별 가격 및 출판 연도 수집하기 ]
        ...
        for i in range(len(all_books)) :
            ...
            overall_price += int(all_books[i]['priceStandard'])
            overall_year += int(all_books[i]['pubDate'][:4])
        overall_avg_price = overall_price/len(all_books)
        overall_avg_year = overall_year/len(all_books)
    # 계산 결과 출력
